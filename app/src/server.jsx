...
app.get('*', (req, res) => {
  ...
  Promise.all(promises).then(() => {
    ...
    const params = {
      ...
      vendorstyle: fs.existsSync(path.resolve('app/compiled/styles/vendor.css'))
      ...
      css: fs.existsSync(path.resolve('app/compiled/styles/client.css'))
        ...
        : null,
      script: fs.existsSync(path.resolve('app/compiled/scripts/client.js'))
        ? `/scripts/client.${md5file.sync(
            './app/compiled/scripts/client.js'
          )}.js`
        : null,
      vendor: fs.existsSync(path.resolve('app/compiled/scripts/vendor.js'))
        ? `/scripts/vendor.${md5file.sync(
            './app/compiled/scripts/vendor.js'
          )}.js`
        : null,
      polyfill: fs.existsSync(path.resolve('app/compiled/scripts/polyfill.js'))
